...
- `supabase/migrations/0019_nearby_workers_require_listing.sql` — **fitur "Pekerja Terdekat"**: sebelumnya fungsi `nearby_workers` menampilkan semua profil ber-role pekerja yang sudah share lokasi, walau belum pernah pasang "Mencari Kerja". Sekarang hanya pekerja dengan postingan "Mencari Kerja" aktif (`jobs.posted_by_role = 'worker'`, `stage = 'terbuka'`, `is_active = true`) yang muncul.
- `supabase/migrations/0020_nearby_workers_with_listing.sql` — `nearby_workers` sekarang ikut mengembalikan detail postingan (`job_id`, judul, kategori, harga, estimasi durasi) supaya kartu pekerja terdekat di beranda bisa link langsung ke halaman postingan dan bisa dipesan.
- `supabase/migrations/0021_admin_delete_user_permanent.sql` — fungsi `admin_delete_user_permanent(p_user_id)` untuk fitur "Hapus Permanen" di `/admin/users`. Hanya bisa dipanggil admin; menolak (raise exception) kalau target adalah admin, saldo dompetnya belum Rp 0, atau punya riwayat escrow/rating/order marketplace (harus pakai Tangguhkan untuk kasus itu). Kalau lolos, semua data pengguna (profil, transaksi, chat, notifikasi, listing, dst) ikut terhapus dari database lewat cascade delete pada `auth.users`.
