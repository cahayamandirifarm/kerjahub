-- =========================================================
-- KERJAHUB — MIGRATION 0024: accept_applicant KEMBALIKAN payer_id
-- Jalankan SETELAH 0023.
--
-- BUG LANJUTAN dari 0023: fix di 0023 sudah benar menentukan SIAPA yang
-- BOLEH membayar (lewat RLS/pengecekan di submit_escrow_proof), tapi
-- tombol "Terima Pelamar" di frontend (AcceptButton.tsx) SELALU
-- mengarahkan orang yang klik terima (pembuat postingan) langsung ke
-- halaman pembayaran escrow — padahal untuk postingan jasa/mencari
-- kerja, pembuat postingan BUKAN pihak yang harus bayar.
--
-- FIX: accept_applicant() sekarang mengembalikan payer_id juga (bukan
-- cuma escrow_id), supaya frontend bisa mengecek: kalau yang klik
-- terima BUKAN payer_id, jangan arahkan ke halaman bayar — tampilkan
-- info "menunggu pembayaran dari pihak lain" saja.
-- =========================================================
